...
if visualization_choice == "Sistem Rekomendasi Wisata":
    ...
    # Pilihan input untuk Lokasi, Nama, dan Umur
    user_location = st.selectbox("Pilih Lokasi", user['Location'].unique())
    user_name = st.selectbox("Pilih Nama", user['User_Id'].unique())
    user_age = st.slider("Pilih Umur", min_value=int(user['Age'].min()), max_value=int(user['Age'].max()), value=int(user['Age'].min()))

    # Mendapatkan User_Id yang sesuai dengan nama, lokasi, dan umur
    user_id = user[(user['Location'] == user_location) & (user['Age'] == user_age) & (user['User_Id'] == user_name)]['User_Id'].values
    if len(user_id) > 0:
        user_id = user_id[0]
    else:
        st.write("User dengan nama, lokasi, dan umur tersebut tidak ditemukan.")
        st.stop()
    ...
    st.write(f"Daftar rekomendasi untuk: Lokasi {user_location}, Nama {user_name}, dan Umur {user_age}")
    st.write("===" * 15)
    st.write("----" * 15)
